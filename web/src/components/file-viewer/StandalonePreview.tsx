...
/**
 * Box-with-mouse-pointer icon used for the inspector toggle.
 * Renders a rounded square frame with a cursor arrow inside — same
 * semantic as lucide's `MousePointerSquare`, but composed locally
 * because the installed lucide-react version doesn't export it.
 */
function MousePointerSquareIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
    ...
    >
      ...
      <path d="M7.5 7.5l5 12 1.8-5.2 5.2-1.8L7.5 7.5z" fill="currentColor" stroke="none" />
    </svg>
  )
}
    ...
    <div className="relative h-screen w-screen overflow-hidden bg-neutral-900">
      ...
      <div className="absolute bottom-6 right-6 z-50 flex items-center border border-white/20 bg-neutral-900/95 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
        ...
        <button
          ...
          className="group flex h-10 w-10 items-center justify-center text-neutral-500 transition-colors hover:bg-white/10 hover:text-neutral-200 active:bg-white/5"
          title={t('standalonePreview.refresh')}
          aria-label={t('standalonePreview.refresh')}
        >
        ...
        <button
          ...
          title={t('standalonePreview.inspect')}
          aria-label={inspectorActive ? t('standalonePreview.inspecting') : t('standalonePreview.inspect')}
        >
          <MousePointerSquareIcon className="h-4 w-4" />
        </button>
      </div>
