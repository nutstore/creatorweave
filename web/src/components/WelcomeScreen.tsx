...
export function WelcomeScreen({ onStartConversation }: WelcomeScreenProps) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center bg-white px-4 dark:bg-background">
      <div className="w-full max-w-xl">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="mb-2 text-2xl font-semibold text-primary dark:text-primary-foreground">{t('welcome.title')}</h1>
          <p className="text-sm text-tertiary dark:text-tertiary">{t('welcome.tagline')}</p>
        </div>
        ...
        <div className="relative">
          <textarea
            ...
            rows={3}
            className="focus:border-primary-300 focus:ring-primary-300 w-full resize-none rounded-xl border border bg-muted px-4 py-3 pr-12 text-sm text-primary placeholder:text-tertiary focus:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:border-border dark:bg-card dark:text-primary-foreground dark:placeholder:text-tertiary dark:focus:bg-neutral-800 dark:focus-visible:ring-offset-neutral-900"
            disabled={!hasApiKey}
          />
          <button
            type="button"
        ...
        <div className="mt-6 flex items-center justify-center gap-4">
          {!directoryHandle && (
            <button
              ...
              className="flex h-8 items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-3 py-1 text-xs font-normal text-secondary transition-colors hover:bg-primary-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-1 dark:border-border dark:bg-card dark:text-muted dark:hover:bg-muted"
            >
