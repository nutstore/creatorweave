...
export class WebContainerRuntimeService {
  ...
  async installDependencies(): Promise<void> {
    ...
    // Some Next.js + Tailwind setups in WebContainer miss optional native binary.
    // Install it explicitly to avoid: Cannot find module '../lightningcss.linux-x64-gnu.node'
    const isNextProject =
      this.currentProjectInfo.startScriptCommand.toLowerCase().includes('next')
    if (isNextProject) {
      try {
        this.emitLog('[install] resolving lightningcss version...')
        let lightningcssVersion = ''
        const versionProcess = await this.webcontainer.spawn(
          'node',
          [
            '-e',
            "try{const p=require('./node_modules/lightningcss/package.json');process.stdout.write(String(p.version||''));}catch(e){process.exit(2)}",
          ],
          { cwd: this.currentProjectInfo.installWorkingDirectory }
        )
        const versionOutputChunks: string[] = []
        void versionProcess.output.pipeTo(
          new WritableStream({
            write: (data) => {
              versionOutputChunks.push(data)
            },
          })
        )
        const versionExitCode = await versionProcess.exit
        if (versionExitCode === 0) {
          lightningcssVersion = versionOutputChunks.join('').trim()
          if (lightningcssVersion) {
            this.emitLog(`[install] detected lightningcss version: ${lightningcssVersion}`)
          }
        }

        const packageName = lightningcssVersion
          ? `lightningcss-linux-x64-gnu@${lightningcssVersion}`
          : 'lightningcss-linux-x64-gnu'

        this.emitLog(`[install] ensuring lightningcss native binary: ${packageName}`)
        const fixProcess = await this.webcontainer.spawn(
          'npm',
        ...
        void fixProcess.output.pipeTo(
          new WritableStream({
            write: (data) => this.emitLog(data),
          })
        )
        const fixExitCode = await fixProcess.exit
        if (fixExitCode === 0) {
          this.emitLog('[install] lightningcss native binary installed')
        } else {
          this.emitLog(
            `[install] warning: lightningcss native binary install failed (exit ${fixExitCode}), retrying with --force`
          )

          const forceProcess = await this.webcontainer.spawn(
            'npm',
          ...
          void forceProcess.output.pipeTo(
            new WritableStream({
              write: (data) => this.emitLog(data),
            })
          )
          const forceExitCode = await forceProcess.exit
          if (forceExitCode === 0) {
            this.emitLog('[install] lightningcss native binary installed (forced)')
          } else {
            this.emitLog(
              `[install] warning: lightningcss native binary install failed after --force (exit ${forceExitCode})`
            )
          }
        }
      } catch (error) {
        this.emitLog(
          `[install] warning: lightningcss native binary install error: ${
            error instanceof Error ? error.message : String(error)
          }`
        )
      }
