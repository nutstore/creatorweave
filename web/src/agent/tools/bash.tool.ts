...
export const bashDefinition: ToolDefinition = {
  ...
  function: {
    ...
    description: [
      ...
      'Available commands: grep, sed, awk, cat, ls, find, sort, uniq, wc, head, tail, jq, diff, xargs, tr, cut, tee, rg, tree, file, split, rev, etc.',
      'NOT available: git, node, npm, python3, curl, wget, tar, patch.',
      'Supports pipes (|), redirections (>, >>), and command chaining (&&, ||, ;).',
      'Workspace at /workspace/<rootName>/... (e.g. /workspace/myroot/src/app.ts). Default cwd is the first root.',
      'Assets at /assets/... (e.g. cat /assets/report.pdf). Agent files at /agents/... (e.g. cat /agents/default/SOUL.md).',
      'Python writes to /workspace/ (shared OPFS), so bash can read files created by Python and vice versa.',
      'File changes are tracked through the same system as write/edit tools.',
    ].join(' '),
    parameters: {
      type: 'object',
      properties: {
