...
**Important:** Field names must match exactly. `configuration` not `config`, `result.pass_rate` not top-level `pass_rate`.
...
```json
{
  ...
  "output_quality": {
    "A": {"score": 9, "strengths": ["Complete solution"], "weaknesses": ["Minor inconsistency"]},
