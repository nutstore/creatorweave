...
export const useOPFSStore = create<OPFSState>()(
  immer((set, get) => ({
    ...
    initialize: async () => {
      try {
        // PR-B made the active workspace URL-driven: workspace.store.initialize
        // intentionally leaves activeWorkspaceId null, and syncFromRoute
        // (App.tsx) sets it later from the URL :workspaceId param. So at app
        // startup there may legitimately be no active workspace yet (e.g.
        // landing on /projects). Mirror refresh()'s behavior: clear stale
        // cached data and finish initialized instead of throwing — when a
        // workspace becomes active, switchWorkspace()/refresh() re-populates
        // the store (and opfs.store.refresh() is fired after each switch).
        const { useWorkspaceStore } = await import('./workspace.store')
        const activeWorkspaceId = useWorkspaceStore.getState().activeWorkspaceId
