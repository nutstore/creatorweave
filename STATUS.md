...
## 19. Next.js App Router migration (2026-08-26, shipped)

> The web app moved from the hand-rolled client router (router/next-router-compat.tsx — a
> matchPath/Route/Routes/HashRouter clone) to real Next.js App Router routing. Branch
> `feat/nextjs-migration`, commits `a650e7a` → `f1cafd3` (2026-08-26).

What landed:

- `app/(app)/` route group: layout with `AppBootstrap` (storage-init state machine + global
  chrome: Toaster, ServiceWorkerBridge, InstallPrompt, ExtensionInstallGuide, PageWriteAuthModal,
  ExecAuthModal, DatabaseRefreshDialog), real pages for `/projects`,
  `/projects/[projectId]/workspaces/[workspaceId]`, docs `[[...path]]` catch-all, `/preview`,
  plus `not-found.tsx` (i18n ×4 locales) and `help/privacy` pages.
- `lib/route-paths.ts` — canonical route table (revived from the deleted `router.ts`);
  navigation goes through `next/navigation` (`useWorkspaceRouteSync` owns the store↔URL sync,
  with tests). View components (`ProjectHomeView`, `WorkspaceRouteView`, `DocsPageView`,
  `PreviewView`) split from their route shells.
...
- Generated `public/` artifacts (`extension/`, `chrome-extension.zip`, `skills/`, `sw.js`)
  are no longer git-tracked — regenerated by dev/build scripts and the Vercel build.
  `public/icons/` is now the canonical tracked icon set (no more copy from the extension).
...
## 20. Agent Bridge (MCP) — WebMCP tools for external CLI agents (2026-08-21, shipped)
