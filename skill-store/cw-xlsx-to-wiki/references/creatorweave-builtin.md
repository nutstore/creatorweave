# CreatorWeave Builtin Integration

## 目标

本文件描述把这个 skill 接入 CreatorWeave 内置 skill 系统时，哪些假设是安全的，哪些假设不应写死在 `SKILL.md` 里。

## 可以依赖的事实

- CreatorWeave 内置 skill 采用 `SKILL.md + scripts/ + references/` 结构
...
- `name` 使用 CreatorWeave 友好的 `cw-` 前缀
...
- 确认 `SKILL.md` frontmatter 兼容 CreatorWeave validator
- 确认没有 `__pycache__` 一类缓存文件
- 确认没有把项目专用路径说明写进通用入口文档
- 确认 `references/` 中的说明与 `scripts/` 实际导出函数一致
- 如果要复制到 CreatorWeave 内置目录，避免把仅用于本仓库本地开发的附属内容一起带入最终 builtin package