...
## Modifications for EO2Weave

This port:

- Replaces the `npx impeccable install` CLI flow with a pure-prompt skill.
- Replaces provider hooks (Claude Code / Cursor / Codex) with no-equivalent — EO2Weave has no PostToolUse hook system, so detector rules are surfaced as design principles for the LLM to apply during review instead of being auto-run on edit.
- Replaces `npx impeccable live` browser iteration with the note that EO2Weave is itself a browser IDE — the user opens files directly.
...
- Frontmatter adapted from Claude Code Skill format to EO2Weave Skill format (name/description/category/tags/triggers).
...
EO2Weave AI assistant, 2026-07-31
