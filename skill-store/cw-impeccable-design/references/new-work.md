...
**CreatorWeave adaptation**: no `concept-seed.mjs` (no Node, no npx). Skip the roll, present 2-3 candidate directions directly with the standing-exit canon option, and let the user pick. Use the structured question tool (CreatorWeave's `ask_user_question`) for the choice. If the user wants to defer, take the canon exit and proceed.

When image generation exists, every card also declares a `sketch` path. In CreatorWeave, no image generation is built-in; surface briefs and design directions carry their identity in palette chips, type samples, and structured prose. That is a complete design direction, not a lesser version.
...
If the work establishes durable strategy for a route or artifact, read its existing surface brief, then update it. In CreatorWeave, surface briefs are project files at `.impeccable/surfaces/<slug>.md` (or in the same directory pattern).
