export const zhCN = {
  ...
  app: {
    ...
    clearFailedCloseOtherTabs: '清空失败：请先关闭该应用的其他标签页/窗口后重试',
    ...
    whatHappened: '发生了什么？',
    databaseHandleInvalidExplanation: '浏览器标签页休眠后，数据库文件句柄失效。这是浏览器的正常行为。',
    ifJustClearedData: '如果刚执行过"清空数据"，请先关闭同源的其他标签页/窗口，再刷新当前页面。',
    ...
    databaseResetExplanation: '这可能由于数据库损坏或迁移失败导致。重置数据库将清除所有数据并重新创建。',
    resetDatabase: '重置数据库',
    reloadPage: '重新加载页面',
  },
  ...
  settings: {
    ...
    syncPanel: {
      ...
      encryptionNotice: '您的会话数据在上传前会被加密。服务器仅存储加密数据，无法访问您的原始内容。',
      ...
      syncDescription: '将当前会话同步到云端，或从云端下载会话。支持端到端加密，仅存储加密数据。',
      loading: '加载中...',
      close: '关闭',
      ...
      conflictResolution: {
        ...
        keepNativeDescription: '保留当前文件系统中的原始版本，放弃 OPFS 中的修改',
        skipThisFile: '跳过此文件',
        ...
        noContent: '无内容',
        fileNotExist: '文件不存在',
        binaryFilePreview: '[{source} 版本为图片或二进制文件，暂不支持文本预览]',
        noReadableContent: '[{source} 版本无可读文本内容]',
        emptyFile: '[{source} 版本为空文件]',
        contentTruncated: '...[内容过长，已截断 {charCount} 字符]',
        whyConflict: '为什么会发生冲突？',
        conflictExplanation: 'OPFS 中的文件在本机文件系统中也被修改了。系统检测到两个版本的修改时间不同，需要您决定保留哪个版本。',
        ifKeepNativeExists: '选择"保留本机版本"将放弃 OPFS 中的修改。',
        ifKeepNativeNotExists: '本机文件不存在，如果选择"保留本机版本"将删除此文件。',
      ...
      syncPreview: {
        ...
        emptyStateDescription: '执行 Python 代码后，检测到的文件系统变更将在此处显示。您可以预览变更详情，然后选择审批通过或拒绝这些变更。',
      ...
      fileDiffViewer: {
        ...
        binaryContent: '二进制内容不支持文本行级 diff，请下载文件或使用专用二进制比对工具。',
        ...
        conversationRunning: '当前会话正在运行，请稍后再试',
        reviewConversation: '变更审阅',
        // Error messages
    ...
    pendingSyncPanel: {
      ...
      rejectedCountWithFailure: '已拒绝 {successCount} 个变更，{failedCount} 个因缺少本地文件基线保留在列表中',
  ...
  skillEditor: {
    ...
    skillContent: '技能内容',
    instruction: '指令',
    instructionPlaceholder: '你是代码审查专家。当用户要求审查代码时：\n1. 分析类型安全性\n2. 检查性能问题\n3. 评估可读性',
    exampleDialog: '示例对话',
    exampleDialogPlaceholder: '用户: "帮我审查这个组件"\nAI: "让我检查一下..."',
    exampleDialogHelp: '可选，提供使用示例帮助 AI 理解',
    outputTemplate: '输出模板',
    outputTemplatePlaceholder: '## 审查报告\n- 文件: {{filename}}\n- 问题: {{issues}}',
  ...
  workflowEditor: {
    ...
    templatePlaceholder: '自定义提示词模板，可使用 {{outputKey}} 引用上游输出...',
  ...
  session: {
    ...
    conversationSwitcher: {
      deleteConfirm: '确定要删除此工作区的缓存吗？所有文件缓存、待同步和撤销记录将被删除。',
  ...
  pendingSync: {
    ...
    failed: '失败',
    skipped: '跳过',
    pendingChangesWillBeWritten: '待同步的更改将写入到真实文件系统。请确保已选择正确的项目文件夹。',
  },
  ...
  conversation: {
    ...
    tokenBudget: '有效输入预算 {effectiveBudget} = 总上限 {modelMaxTokens} - 预留 {reserveTokens}',
    // 空状态
    empty: {
      title: '开始新的对话',
    ...
    usage: {
      ...
      nearLimit: '接近上限',
      comfortable: '宽裕',
      tokenUsage: '输入 {promptTokens} + 输出 {completionTokens} = {totalTokens} tokens',
    },
  },
  ...
  conversationStorage: {
    ...
    selectScope: '选择清理范围',
    cleanupOldSessions: '仅清理旧会话 (30天未活跃)',
    cleanupAll: '清理所有对话缓存',
    ...
    browserQuota: '(浏览器配额)',
    loading: '加载中...',
    quotaExplanation: '配额是浏览器允许的上限，不等于实际剩余空间。写入时若超出实际空间会报错。',
  ...
  errorBoundary: {
    renderError: '渲染错误',
    componentRenderError: '组件渲染时发生错误。这可能是临时问题，请尝试刷新页面。',
    errorDetails: '错误详情',
    retry: '重试',
  ...
  sidebar: {
    ...
    emptyStateNoWorkspace: '当前项目还没有工作区，创建首个会话后会自动生成对应工作区。',
    ...
    snapshotList: {
      ...
      switchPartial: '切换到快照未完全成功（失败快照 {failedSnapshotId}），仍有 {count} 个文件未恢复',
    ...
    snapshotApproval: {
      title: '创建快照',
      description: '将审批通过 <span class="font-semibold">{count}</span> 个变更，并创建快照记录。',
      summaryLabel: '快照描述',
      generateAI: 'AI 生成',
      generating: '生成中...',
