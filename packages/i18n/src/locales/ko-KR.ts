export const koKR = {
  ...
  app: {
    ...
    // App toast messages
    resetDatabaseFailed: '데이터베이스 재설정에 실패했습니다. 페이지를 수동으로 새로고침해 주세요',
    localDataCleared: '로컬 데이터가 삭제되었습니다. 처음부터 다시 시작할 수 있습니다',
    clearFailedCloseOtherTabs: '삭제에 실패했습니다. 먼저 이 앱의 다른 탭/창을 닫은 후 다시 시도해 주세요',
    clearLocalDataFailed: '로컬 데이터 삭제에 실패했습니다',
    storageInitError: '스토리지 초기화 오류',
    projectNotFound: '프로젝트를 찾을 수 없거나 삭제되었습니다',
    switchProjectFailed: '프로젝트 전환에 실패했습니다. 나중에 다시 시도해 주세요',
    noWorkspaceInProject: '현재 프로젝트에 작업 공간이 없습니다',
    projectCreated: '프로젝트 "{name}"이(가) 생성되었습니다',
    projectCreatedButSwitchFailed: '프로젝트가 생성되었지만 전환에 실패했습니다. 수동으로 다시 시도해 주세요',
    createProjectFailed: '프로젝트 생성에 실패했습니다. 나중에 다시 시도해 주세요',
    ...
    databaseHandleInvalidExplanation: '브라우저 탭이 최대 절전 모드 후 데이터베이스 파일 핸들이 무효가 됩니다. 이는 정상적인 브라우저 동작입니다.',
    ifJustClearedData: '"데이터 삭제"를 방금 실행한 경우 먼저 동일한 출처의 다른 탭/창을 닫은 다음 현재 페이지를 새로고침해 주세요.',
    yourDataIsSafe: '대화 데이터는 안전합니다!',
    dataStoredInOPFS: '데이터는 브라우저 OPFS에 저장되어 있으며 일시적으로 액세스할 수 없을 뿐입니다.',
    willAutoRecoverAfterRefresh: '페이지를 새로고침하면 데이터베이스 연결이 자동으로 복원됩니다.',
    refreshPage: '페이지 새로고침',
    cannotCloseDialog: '이 대화상자는 닫을 수 없습니다 - 위 버튼을 클릭하여 페이지를 새로고침해 주세요',
    databaseInitFailed: '데이터베이스 초기화 실패',
    databaseResetExplanation: '이는 데이터베이스 손상 또는 마이그레이션 실패로 인해 발생할 수 있습니다. 데이터베이스를 재설정하면 모든 데이터가 지워지고 다시 생성됩니다.',
  ...
  settings: {
    ...
    syncPanel: {
      ...
      encryptionNotice: '세션 데이터는 업로드 전에 암호화됩니다. 서버는 암호화된 데이터만 저장하며 원본 콘텐츠에 액세스할 수 없습니다.',
      ...
      confirmDelete: '이 동기화 세션을 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.',
      crossDeviceSync: '기기 간 동기화',
      syncDescription: '현재 세션을 클라우드에 동기화하거나 클라우드에서 세션을 다운로드합니다. 종단 간 암호화를 지원하며 암호화된 데이터만 저장됩니다.',
      loading: '로딩 중...',
      close: '닫기',
      ...
      conflictResolution: {
        ...
        noReadableContent: '[{source} 버전에 읽을 수 있는 텍스트 내용이 없습니다',
        emptyFile: '[{source} 버전은 빈 파일입니다',
        contentTruncated: '...[내용이 너무 길어서 {charCount}자 삭제됨]',
        whyConflict: '왜 충돌이 발생했나요?',
        conflictExplanation: 'OPFS의 파일이 로컬 파일 시스템에서도 수정되었습니다. 시스템이 두 버전의 수정 시간이 다름을 감지했고, 어떤 버전을 유지할지 결정해야 합니다.',
        ifKeepNativeExists: '"로컬 버전 유지"를 선택하면 OPFS의 변경이 취소됩니다.',
        ifKeepNativeNotExists: '로컬 파일이 존재하지 않습니다. "로컬 버전 유지"를 선택하면 이 파일이 삭제됩니다.',
        skipThisConflict: '이 충돌 건너뛰기',
        applySelection: '선택 적용',
        nativeNotConnected: '[로컬 디렉터리가 연결되어 있지 않아 로컬 버전을 읽을 수 없습니다]',
      },

      // Sync Preview Panel (Empty State)
      syncPreview: {
        emptyStateTitle: '변경 검토 대기 중',
        emptyStateDescription: 'Python 코드 실행 후 감지된 파일 시스템 변경 사항이 여기에 표시됩니다. 변경 세부 정보를 미리 보고 승인을 거부할 수 있습니다.',
      ...
      fileChangeList: {
        ...
        noChangesDescription: 'Python 실행 후 파일 시스템 변경이 감지되지 않았습니다',
        added: '추가',
        modified: '수정',
        deleted: '삭제',
      ...
      fileDiffViewer: {
        ...
        selectFileHint: '왼쪽 목록에서 파일을 선택하여 버전과 현재 파일의 차이를 확인하세요',
        ...
        binaryContent: '바이너리 내용은 텍스트 레벨 차이를 지원하지 않습니다. 파일을 다운로드하거나 전용 바이너리 비교 도구를 사용하세요.',
        ...
        close: '닫기',
        // AI review prompt
        reviewPromptIntro: '다음 파일 스냅샷을 검토하고 수정 제안을 제공해 주세요:',
        file: '파일',
        changeType: '변경 유형',
        snapshot: '스냅샷',
        recordedAt: '기록 시간',
        ...
        conversationRunning: '현재 대화 실행 중입니다. 나중에 다시 시도해 주세요',
        reviewConversation: '변경 검토',
        // Error messages
        loadFailedError: '파일 불러오기 실패',
        cannotReadNativeContent: '네이티브 파일 내용을 보려면 프로젝트 디렉토리를 선택하세요',
    ...
    pendingSyncPanel: {
      ...
      confirmRejectMessage: '모든 변경을 거부하시겠습니까? 이 작업은 취소할 수 없습니다.',
      cancel: '취소',
      confirmReject: '거부 확인',
      reviewSuccess: '검토 성공!',
      rejectedAllSuccess: '모든 변경이 거부되었습니다',
      rejectedCountWithFailure: '{successCount}개 변경을 거부했고, {failedCount}개는 로컬 파일 베이스라인이 없어서 목록에 유지됩니다',
      ...
      conversationRunningPleaseWait: '현재 대화가 실행 중입니다. 나중에 다시 시도하세요',
    ...
    modelSelection: {
      ...
      customModelHint: '활성화하면任意の 모델 이름을 입력할 수 있으며, 새로 출시된 모델에 적합합니다',
    },
  ...
  welcome: {
    ...
    apiKeyRequiredHint: '먼저 모델 설정에서 API Key를 구성한 후 대화를 시작하세요',
  ...
  skillEditor: {
    ...
    skillContent: '스킬 내용',
    instruction: '지시',
    instructionPlaceholder: '당신은 코드 리뷰 전문가입니다. 사용자가 코드 리뷰를 요청할 때：\n1. 타입 안전성 분석\n2. 성능 문제 확인\n3. 가독성 평가',
    exampleDialog: '예제 대화',
    exampleDialogPlaceholder: '사용자: "이 컴포넌트 리뷰帮我"\nAI: "확인하겠습니다..."',
    exampleDialogHelp: '선택 사항, AI 이해를 돕는 예제 제공',
    outputTemplate: '출력 템플릿',
    outputTemplatePlaceholder: '## 리뷰 보고서\n- 파일：{{filename}}\n- 문제：{{issues}}',
  ...
  webContainer: {
    ...
    // Config section
    startupConfig: '시작 구성',
    startupConfigHelp: '모노레포 또는 멀티 앱 디렉터리 구조를 지원하기 위해 하위 디렉터리와 스크립트를 선택할 수 있습니다.',
    directorySelect: '디렉터리',
    selectDirectory: '디렉터리 선택',
    currentStartupDir: '현재 시작 디렉터리',
    dirChangeRequiresRestart: '디렉tery를 변경하면 다시 시작하거나 다시 시작해야 적용됩니다',
  ...
  workflowEditor: {
    ...
    clickCanvasNode: '캔버스의 노드를 클릭하거나 오른쪽에서 새 노드를 추가하세요',
    kind: '유형',
    role: '역할',
    outputKey: '출력 키',
    ...
    templatePlaceholder: '사용자 정의 프롬프트 템플릿, {{outputKey}}를 사용하여 업스트림 출력 참조...',
  ...
  customWorkflowManager: {
    ...
    clickToCreateFirst: '「새 워크플로우」를 클릭하여 첫 번째 워크플로우를 만드세요',
    // Domain labels
    generic: '범용',
    novel: '소설 작성',
    video: '비디오 스크립트',
    ...
    // Delete dialog
    confirmDelete: '삭제 확인',
    deleteConfirmMessage: '워크플로우 "{name}"을(를) 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.',
    cancel: '취소',
    delete: '삭제',
    // Footer
  ...
  workflowEditorDialog: {
    ...
    workflow: '워크플로우',
    // Confirm dialog
    unsavedChangesConfirm: '저장되지 않은 변경 사항이 있습니다. 템플릿을 전환하시겠습니까?',
    // Status
    valid: '유효',
    errors: '{count}개 오류',
    // Actions
  ...
  session: {
    ...
    conversationSwitcher: {
      deleteConfirm: '이 작업 영역 캐시를 삭제하시겠습니까? 모든 파일 캐시, 보류 중인 동기화, 실행 취소 기록이 삭제됩니다.',
  ...
  pendingSync: {
    ...
    failed: '실패',
    skipped: '건너뜀',
    pendingChangesWillBeWritten: '보류 중인 변경사항은 실제 파일시스템에 기록됩니다. 올바른 프로젝트 폴더를 선택했는지 확인하세요.',
  },
  ...
  conversation: {
    ...
    toolCall: '도구 호출',
    regenerate: '재생성',
    regenerateConfirmMessage: '이 메시지를 다시 보내시겠습니까? 현재 답장이 대체됩니다.',
    ...
    tokenBudget: '유효 입력 예산 {effectiveBudget} = 총 한도 {modelMaxTokens} - 예약 {reserveTokens}',
    empty: {
      title: '새 대화 시작',
    ...
    usage: {
      ...
      nearLimit: '한계 근접',
      comfortable: '여유 있음',
      tokenUsage: '입력 {promptTokens} + 출력 {completionTokens} = {totalTokens} tokens',
    },
  },
  ...
  conversationStorage: {
    ...
    cleanupSuccess: '{count}개의 대화 파일 캐시를 정리하여 {size}를 확보했습니다',
    ...
    cleanupHelpText: '대화 기록은 삭제되지 않습니다. 다음에 파일에 접근할 때 디스크에서 다시 읽어옵니다.',
    canceling: '취소',
    cleaning: '정리 중...',
    ...
    loading: '로딩 중...',
    quotaExplanation: '할당량은 브라우저 허용 한도이며 실제 남은 공간과 다릅니다. 실제 공간을 초과하여 쓰기하면 오류가 발생합니다.',
    ...
    cleanupFileCacheHelp: '파일 캐시만 정리하며 대화 기록에는 영향을 주지 않습니다',
  },
  ...
  offlineQueue: {
    ...
    tasksSavedAutomatically: '네트워크 중단 시 작업이 자동으로 대기열에 저장됩니다',
  },
  ...
  errorBoundary: {
    renderError: '렌더링 오류',
    componentRenderError: '컴포넌트 렌더링 중 오류가 발생했습니다. 일시적인 문제일 수 있으므로 페이지를 새로 고쳐주세요.',
    errorDetails: '오류 상세',
    retry: '재시도',
  ...
  fileTree: {
    ...
    emptyStateDescription: 'Pure OPFS 샌드박스 모드에서는 파일 변경 사항이 여기에 표시됩니다',
    draftFiles: '임시 파일',
  ...
  sidebar: {
    ...
    emptyStateNoWorkspace: '이 프로젝트의 워크스페이스가 아직 없습니다. 첫 대화를 시작하면 워크스페이스가 자동으로 생성됩니다.',
    ...
    snapshotList: {
      ...
      switchPartial: '스냅샷 전환이 완전히 성공하지 못했습니다(실패한 스냅샷 {failedSnapshotId}), {count}개 파일이 아직 복원되지 않았습니다',
      switchFailed: '전환이 실패하고 자동 복구도 완전히 성공하지 못했습니다. 스냅샷 상태를 수동으로 확인하세요',
      switchFailedWithCount: '최신으로의 전환이 완전히 성공하지 못했습니다, {count}개 파일이 아직 복원되지 않았습니다',
      ...
      confirmClearMessage: '이 프로젝트의 모든 스냅샷을 지우시겠습니까? 이 작업은 취소할 수 없습니다.',
      confirmDeleteTitle: '삭제 확인',
      confirmDeleteMessage: '이 스냅샷을 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.',
      approved: '승인됨',
      committed: '커밋됨',
      draft: '초안',
      rolledBack: '롤백됨',
    ...
    snapshotApproval: {
      title: '스냅샷 생성',
      description: '<span class="font-semibold">{count}</span>개 변경을 승인하고 스냅샷 레코드를 생성합니다.',
      summaryLabel: '스냅샷 설명',
      generateAI: 'AI 생성',
      generating: '생성 중...',
      summaryPlaceholder: '스냅샷 설명 입력 (여러 줄 가능, 첫 줄을 제목으로 사용)',
    ...
    confirmClearWorkspace: '현재 프로젝트의 모든 워크스페이스를 지우시겠습니까? 이 작업은 취소할 수 없습니다.',
