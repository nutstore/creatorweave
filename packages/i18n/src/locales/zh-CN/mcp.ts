export const mcp = {
    dialog: {
      title: "MCP 服务配置",
    },
    title: "MCP 服务器",
    description: "管理外部 MCP 服务连接",
    extensionRequired: "需要安装浏览器扩展",
    extensionRequiredHint: "MCP 服务连接需要安装 怡氧知知 浏览器扩展。请在设置的「扩展」标签页中安装。",
    addServer: "添加 MCP 服务器",
    editServer: "编辑服务器",
    add: "添加",
    update: "更新",
    saving: "保存中...",
    toolsCount: "{count} 个工具",
    confirmDelete: "确定要删除此 MCP 服务器吗？",
    badge: {
      builtin: "内置",
      disabled: "已禁用",
    },
    empty: {
      title: "暂无 MCP 服务器",
      hint: "点击上方按钮添加服务器",
    },
    connectAll: "全部连接",
    export: "导出服务器配置",
    import: "导入服务器配置",
    actions: {
      clickToDisable: "点击禁用",
      clickToEnable: "点击启用",
      connect: "连接",
      reconnect: "重新连接",
      disconnect: "断开",
      editConfig: "编辑配置",
      deleteServer: "删除服务器",
    },
    status: {
      connected: "已连接",
      connecting: "连接中…",
      error: "连接错误",
      disconnected: "未连接",
    },
    toolsLabel: "个工具",
    toast: {
      loadFailed: "加载 MCP 服务器失败",
      updated: "服务器配置已更新",
      added: "服务器已添加",
      saveFailed: "保存失败",
      deleted: "服务器已删除",
      deleteFailed: "删除失败",
      updateStatusFailed: "更新状态失败",
      connected: "连接成功",
      connectFailed: "连接失败",
      disconnected: "已断开连接",
      disconnectFailed: "断开连接失败",
      noServersToConnect: "没有可连接的服务器",
      connectingAll: "正在连接 {count} 个服务器…",
      allConnected: "全部连接完成",
      exported: "已导出 {count} 个服务器",
      imported: "已导入 {imported} 个，跳过 {skipped} 个",
      invalidFormat: "格式错误：需要数组格式",
      importFailed: "导入文件解析失败",
    },
    validation: {
      invalidServerId: "无效的服务器 ID",
      nameRequired: "请输入服务器名称",
      urlRequired: "请输入服务器 URL",
      urlInvalid: "请输入有效的 URL",
      timeoutRange: "超时时间应在 1000-300000ms 之间",
      serverIdExists: "服务器 ID 已存在",
      serverIdValid: "ID 格式正确",
    },
    form: {
      serverId: "服务器 ID",
      serverIdPlaceholder: "如: excel-analyzer",
      serverIdHint: "用于工具调用，如: excel-analyzer:analyze_spreadsheet",
      displayName: "显示名称",
      displayNamePlaceholder: "如: Excel 文档分析器",
      description: "描述",
      descriptionPlaceholder: "服务器功能描述",
      serverUrl: "服务器 URL",
      transportType: "传输类型",
      authTokenOptional: "认证 Token（可选）",
      timeoutMs: "超时时间 (ms)",
      transport: {
        sse: "SSE (Server-Sent Events)",
        streamableHttp: "Streamable HTTP",
        streamableHttpExperimental: "Streamable HTTP (实验性)",
      },
    },
} as const
