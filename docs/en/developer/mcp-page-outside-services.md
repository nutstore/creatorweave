...
This guide explains how to connect **out-of-page MCP services** to CreatorWeave, including:
...
> All paths are relative to the repository root `creatorweave/`.
...
- Config location: `web/src/mcp/preset-providers.ts`
- UI entry: `web/src/components/mcp/MCPSettings.tsx`
...
Mail MCP is a CreatorWeave-built streamable_http MCP server providing email drafting / sending / draft management. Typical flow: the agent drafts an email for the user → review → confirm → send.
...
3. The user adds the Mail MCP preset in CreatorWeave MCP Settings → pastes the same token into Auth Token
4. The agent calls `tools/call` → the server identifies the user by token → sends via that user's SMTP credentials

**Key point**: CreatorWeave does **not** store SMTP credentials — only the Personal Mail Token (generated by mail-mcp itself). The server looks up account config by token and decrypts the SMTP password. The CreatorWeave frontend never needs to know SMTP details.
...
- Config location: `web/src/mcp/preset-providers.ts` (id: `mail`, category: `communication`, icon: `mail`)
- UI entry: `web/src/components/mcp/MCPSettings.tsx`
...
- `mail-mcp` already reserves `draft_response.preview.text/html` fields (in draft tool JSON responses) for a future confirmation dialog, but CreatorWeave does not consume them yet
