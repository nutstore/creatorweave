// ============================================================
// Browser-side Selector Synthesis
//
// Exports `synthesizeElementLocators(el)` — a self-contained function
// that produces a ranked list of CSS / link_text / id / name locators
// for the given element, each with a stability score and verification
// info.
//
...
//
// History: this module used to export a JS *source string* that the MAIN
// world content script evaluated via `new Function(...)`. Strict-CSP pages
// (no 'unsafe-eval') blocked that evaluation, breaking synthesize_locators
// on sites like GitHub / Google. The logic is now a direct module export —
// normal function calls are NOT restricted by page CSP.
//
...
export type SynthesizedLocator = {
  kind: string
  query: string
  verification: string
  stability: string
...
type PathSegment = { tag: string; index: number }
...
export function synthesizeElementLocators(el: Element): SynthesizedLocator[] {
  if (!(el instanceof Element)) return []

  const PRIMARY_ATTRIBUTES = [
    'data-testid', 'data-test', 'data-qa', 'aria-label', 'title',
    'placeholder', 'alt', 'name', 'role', 'type', 'value',
  ]
  ...
  const cssIdent = (value: unknown): string => {
  ...
  const isConcreteTag = (tag: string): boolean => /^[a-z][a-z0-9_-]*$/.test(tag)

  const tagOf = (el: Element | null | undefined): string => {
    if (!(el instanceof Element)) return '*'
    const tag = normalize(el.tagName).toLowerCase()
    return isConcreteTag(tag) ? tag : '*'
  }

  const getTraversalParent = (el: Element): Element | null => {
    if (!(el instanceof Element)) return null
    if (el.parentElement instanceof Element) return el.parentElement
    const root = typeof el.getRootNode === 'function' ? el.getRootNode() : null
    if (root && root !== document && (root as ShadowRoot).host instanceof Element) {
      return (root as ShadowRoot).host
    }
    return null
  }

  const getTagIndex = (el: Element): number => {
  ...
  const getPath = (el: Element): PathSegment[] => {
  ...
  const pathQuery = (segments: PathSegment[], includeFirstIndexes: boolean): string =>
    segments
      .map((segment) =>
        !includeFirstIndexes && segment.index === 1
          ? segment.tag
          : segment.tag + ':nth-of-type(' + segment.index + ')')
      .join(' > ')

  const countCssMatches = (selector: string): number => document.querySelectorAll(selector).length
  const firstCssMatch = (selector: string): Element | null => document.querySelector(selector)
  const verifyCss = (el: Element, selector: string): boolean =>
    countCssMatches(selector) === 1 && firstCssMatch(selector) === el

  const getClassTokens = (el: Element): string[] => {
    if (!(el instanceof Element)) return []
  ...
  const combinations = (values: string[], size: number): string[][] => {
    const result: string[][] = []
    const walk = (start: number, bucket: string[]) => {
  ...
  const attrSelector = (tag: string, attrName: string, attrValue: string): string =>
    tag + '[' + attrName + '="' + cssValue(attrValue) + '"]'

  const classSelector = (tag: string, group: string[]): string =>
    tag + group.map((token) => '.' + cssIdent(token)).join('')

  const simpleFragments = (el: Element): SimpleFragment[] => {
    const tag = tagOf(el)
    const fragments: SimpleFragment[] = []
    if (tag !== '*') {
      fragments.push({
        query: tag,
        verification: 'tag',
    ...
    const classTokens = getClassTokens(el)
      .sort((left, right) => {
  ...
  const uniqueAnchorFragments = (ancestor: Element, depth: number): SimpleFragment[] => {
    const fragments: SimpleFragment[] = []
  ...
  const relativePathFromAncestor = (target: Element, ancestor: Element): PathSegment[] => {
    const targetPath = getPath(target)
    const ancestorPath = getPath(ancestor)
    return targetPath.slice(ancestorPath.length)
  }

  // ---- main body (previously the returned closure) ----
  const locators: SynthesizedLocator[] = []
  const seen = new Set<string>()
  const tag = tagOf(el)
